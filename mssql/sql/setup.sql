...
BEGIN
    CREATE TABLE [Mailbox] (
        ...
        CONSTRAINT [PK_Mailbox] PRIMARY KEY ([MailboxId])
    );
END;
...
BEGIN
    CREATE TABLE [Mail] (
        ...
        [RawEmail] nvarchar(max) NULL,
        [MailboxId] uniqueidentifier NOT NULL,
        [Status] int NOT NULL,
        CONSTRAINT [PK_Mail] PRIMARY KEY ([MailId]),
        CONSTRAINT [FK_Mail_Mailbox_MailboxId] FOREIGN KEY ([MailboxId]) REFERENCES [Mailbox] ([MailboxId]) ON DELETE CASCADE
    );
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20200912141336_MailboxAndMailTables')
BEGIN
    CREATE INDEX [IX_Mail_MailboxId] ON [Mail] ([MailboxId]);
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Mail]') AND [c].[name] = N'Body');
    IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [Mail] DROP CONSTRAINT [' + @var4 + '];');
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Mail]') AND [c].[name] = N'RawEmail');
    IF @var5 IS NOT NULL EXEC(N'ALTER TABLE [Mail] DROP CONSTRAINT [' + @var5 + '];');
    ALTER TABLE [Mail] DROP COLUMN [RawEmail];
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20200916185536_MailImprovements')
BEGIN
    ALTER TABLE [Mail] ADD [RawEmail] varbinary(max) NULL;
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20200916185536_MailImprovements')
BEGIN
    ALTER TABLE [Mail] ADD [HtmlBody] nvarchar(max) NULL;
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20200916185536_MailImprovements')
BEGIN
    ALTER TABLE [Mail] ADD [TextBody] nvarchar(max) NULL;
END;
